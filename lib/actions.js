...
function parsePerfumeForm(formData) {
  ...
  return { name, price, imageUrl, videoUrl, description, category, comparePrice, featured, notes };
}
...
function parseExpenseForm(formData) {
  ...
  return { description, amount, note, paidWith };
}
...
export async function registerCashMovementAction(prevState, formData) {
  ...
  if (!['aporte', 'retiro'].includes(kind)) return { error: 'Elige si es un aporte o un retiro.' };
  ...
  try {
    ...
    await createCashMovement({ kind, amount, note, occurredAt: date ? `${date}T12:00:00-05:00` : null });
  } catch (error) {
    return { error: error.message };
  }
  revalidatePath('/admin');
