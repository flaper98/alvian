...
function parsePerfumeForm(formData) {
  ...
  return { name, price, imageUrl, videoUrl, description, category };
}
...
export async function updatePanderoEntryNameAction(id, prevState, formData) {
  await requireRole('admin', 'vendedora');
  ...
  try {
    await updatePanderoEntryName(id, customerName);
  } catch (error) {
    return { error: error.message };
  }
