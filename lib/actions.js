...
export async function registerPurchaseAction(prevState, formData) {
  ...
  try {
    await createPurchase({ perfumeId, quantity, unitCost, note });
  } catch (error) {
    return { error: error.message };
  }
...
export async function registerSaleAction(prevState, formData) {
  ...
  if (!['contado', 'credito'].includes(paymentType)) {
    return { error: 'Selecciona una forma de pago.' };
  }
  if (paymentType === 'credito' && !customerName) {
    return { error: 'El nombre del cliente es obligatorio para ventas a crédito.' };
  }
