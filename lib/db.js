...
export async function updateSale(id, { quantity, unitPrice, paymentType, customerName, soldByRole }) {
  ...
  try {
    ...
    const { rows: existingRows } = await client.query(
      'SELECT perfume_id, quantity, unit_price, payment_type, sold_by_role, sold_by_name FROM sales WHERE id = $1 FOR UPDATE',
      [id],
    );
    ...
    // Si se reasigna el rol de quien vendió, ya no sabemos el nombre exacto de la
    // nueva persona (el formulario solo elige el rol), así que se limpia el nombre
    // guardado y la vista vuelve a mostrar la etiqueta genérica del rol.
    const soldByName = existing.sold_by_role === soldByRole ? existing.sold_by_name : null;
