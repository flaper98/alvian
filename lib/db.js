...
export async function getSummary(role) {
    ...
    expensesTotal,
      ...
      pool.query(`
        SELECT
          ...
          (SELECT COALESCE(SUM(cuotas_total), 0) FROM pandero_groups)
            + (SELECT COALESCE(SUM(amount), 0) FROM pandero_round_payments) AS locked_total
      `),
  ...
  return {
    ...
    // cobrado y cerrado de semanas anteriores (pagos guardados por número en
    // pandero_round_payments, más cuotas_total de las rondas que se cerraron
    // antes de existir ese historial) más lo marcado "Pagando" en la semana
  ...
  try {
    ...
    if (entry) {
      ...
      if (!entry.round_recorded) {
        await client.query('SELECT id FROM perfumes WHERE id = $1 FOR UPDATE', [entry.perfume_id]);
