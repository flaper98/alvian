...
export async function updatePerfume(id, { name, price, imageUrl, videoUrl, description, category, stock }) {
  await ensureSchema();
  ...
  const { rows } = await getPool().query(
    `UPDATE perfumes
     ...
     WHERE id = $7
     RETURNING id, name, price, image_url, video_url, description, stock, category, created_at`,
    [name, price, imageUrl, videoUrl || null, description || null, category || null, id, stock ?? null],
  );
  return rows[0];
}
