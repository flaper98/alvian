...
export async function setPanderoEntryFulfilled(id, fulfilled) {
  ...
  try {
    ...
    const { rows: entryRows } = await client.query(
      'UPDATE pandero_entries SET fulfilled = true WHERE id = $1 RETURNING id, fulfilled, group_id',
      [id],
    );
...
export async function updatePanderoEntryName(id, customerName) {
  await ensureSchema();
  const { rows } = await getPool().query(
    'UPDATE pandero_entries SET customer_name = $1 WHERE id = $2 RETURNING id, customer_name',
    [customerName, id],
  );
  return rows[0];
}
...
export async function listPanderoGroups() {
  ...
  const { rows: entries } = await getPool().query(`
    SELECT e.id, e.group_id, e.position, e.customer_name, e.fulfilled, e.paying, e.created_at,
           f.name AS perfume_name,
