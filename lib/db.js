 ...
 * precio, emparejando cada producto contra el catálogo ya existente. Devuelve
 * qué se guardó, qué producto no existe en el catálogo, y qué línea fue
 * ambigua (coincide con más de un perfume). No lanza error: siempre guarda lo
 * que sí pudo emparejar. */
export async function bulkUpsertSupplierPrices({ supplierId, tierLabel, rawText }) {
  ...
  for (const line of lines) {
    ...
    if (!split) {
      unmatched.push({ line, reason: 'No se encontró un precio en la línea.' });
      continue;
    }
    ...
    if (!productRaw || Number.isNaN(price) || price <= 0) {
      unmatched.push({ line, reason: 'No se pudo leer el producto o el precio.' });
      continue;
    }
    ...
    if (!match) {
      unmatched.push({ line, reason: `"${productRaw}" no está en tu catálogo.` });
      continue;
    }
...
export async function listSupplierPrices(supplierId) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `SELECT sp.id, sp.perfume_id, sp.tier_label, sp.price, sp.created_at, f.name AS perfume_name
     FROM supplier_prices sp
     JOIN perfumes f ON f.id = sp.perfume_id
     WHERE sp.supplier_id = $1
     ORDER BY f.name ASC, sp.price ASC`,
    [supplierId],
  );
  return rows;
}
...
export async function listPriceComparison() {
  await ensureSchema();
  const { rows } = await getPool().query(`
    SELECT sp.perfume_id, f.name AS perfume_name, sp.id, sp.tier_label, sp.price,
  ...
  for (const row of rows) {
    if (!byPerfume.has(row.perfume_id)) {
      byPerfume.set(row.perfume_id, { perfumeId: row.perfume_id, perfumeName: row.perfume_name, options: [] });
    }
    byPerfume.get(row.perfume_id).options.push({
      id: row.id,
  ...
  return Array.from(byPerfume.values());
}
