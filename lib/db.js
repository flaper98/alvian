...
export async function createExpense({ description, amount, note, paidWith }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `INSERT INTO expenses (description, amount, note, paid_with)
     VALUES ($1, $2, $3, $4)
     RETURNING id, description, amount, note, paid_with, created_at`,
    [description, amount, note || null, paidWith],
  );
  return rows[0];
}
...
export async function listExpenses() {
  await ensureSchema();
  const { rows } = await getPool().query(
    'SELECT id, description, amount, note, paid_with, created_at FROM expenses ORDER BY created_at DESC',
  );
  return rows;
}

export async function updateExpense(id, { description, amount, note, paidWith }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `UPDATE expenses SET description = $1, amount = $2, note = $3, paid_with = $4
     WHERE id = $5
     RETURNING id, description, amount, note, paid_with, created_at`,
    [description, amount, note || null, paidWith, id],
  );
  return rows[0];
}
...
export async function getCashFlow({ start = null, end = null } = {}) {
  ...
  const [contado, creditPayments, closedRounds, legacyCuotas, paying, purchases, expenses, commissions, movements] =
    await Promise.all([
      pool.query(
  ...
  const earningsOut = purchasesReinvested + expensesFromEarnings + commissionsPaid + withdrawals;
  ...
  return {
    ...
    capitalIn,
    capitalPut: purchasesCapital + expensesCapital + capitalIn,
