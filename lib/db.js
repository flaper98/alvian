...
/**
 * Todo el dinero que entró y salió del negocio en un rango de fechas.
 * Lo cobrado de ventas excluye el "cobro automático" que se genera al entregar
 * un turno del pandero: ese dinero ya entró antes con las cuotas semanales y
 * contarlo otra vez lo duplicaría.
 */
export async function getCashFlow({ start = null, end = null } = {}) {
  ...
  const [contado, creditPayments, cuotas, legacyCuotas, paying, purchases, expenses, commissions, movements] =
    await Promise.all([
      pool.query(
      ...
      pool.query(
        `SELECT COALESCE(SUM(amount), 0) AS total FROM pandero_round_payments WHERE ${inRange('created_at')}`,
        range,
      ),
      // Cuotas de rondas cerradas antes de existir el historial: no tienen fecha.
      allTime
        ? pool.query('SELECT COALESCE(SUM(cuotas_total), 0) AS total FROM pandero_groups')
        : Promise.resolve({ rows: [{ total: 0 }] }),
      // Cuotas marcadas "Pagando" en la semana actual (ya se cobraron, aún sin cerrar la ronda).
      includesNow
  ...
  const collectedSales = Number(contado.rows[0].total) + Number(creditPayments.rows[0].total);
  const panderoCuotas =
    Number(cuotas.rows[0].total) +
    Number(legacyCuotas.rows[0].total) +
    paying.rows[0].count * PANDERO_CUOTA_AMOUNT;
  ...
  const incomeTotal = collectedSales + panderoCuotas;
  const outTotal = purchasesTotal + expensesTotal + commissionsPaid + withdrawals;
  // Las compras se consideran pagadas primero con lo que aportaste en el
  // mismo período; el resto salió de lo que entró por ventas (reinversión).
  const purchasesFromCapital = Math.min(purchasesTotal, capitalIn);
  const purchasesFromSales = purchasesTotal - purchasesFromCapital;
  ...
  return {
    ...
    purchasesFromCapital,
    purchasesFromSales,
    reinvestRatePct: incomeTotal > 0 ? (purchasesFromSales / incomeTotal) * 100 : null,
  };
}
...
export async function getSummary(role, period = 'mes') {
  ...
  const [
    ...
    pool.query(
      `SELECT payment_type, COUNT(*)::int AS count, COALESCE(SUM(quantity * unit_price), 0) AS total
       FROM sales WHERE ${inRange('created_at')} GROUP BY payment_type`,
      range,
    ),
    pool.query(
    ...
    start || end ? getCashFlow() : null,
  ]);

  const byType = (type) => byPaymentType.rows.find((row) => row.payment_type === type);
  ...
  const netProfit = grossProfit - commissionsEarned - flow.expenses;
  ...
  return {
    ...
    salesTotal,
    contadoCount: byType('contado')?.count || 0,
    contadoTotal: Number(byType('contado')?.total || 0),
    creditoCount: byType('credito')?.count || 0,
    creditoTotal: Number(byType('credito')?.total || 0),
    panderoCount: byType('pandero')?.count || 0,
    panderoTotal: Number(byType('pandero')?.total || 0),
    estimatedCost,
    grossProfit,
    commissionsEarned,
    expensesTotal: flow.expenses,
    netProfit,
    profitMarginPct: salesTotal > 0 ? (netProfit / salesTotal) * 100 : 0,
    flow,
    cashOnHand: (cashOnHandFlow || flow).net,
