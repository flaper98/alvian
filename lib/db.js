import { Pool } from 'pg';
import { PANDERO_CUOTA_AMOUNT } from './pandero';
...
export async function listPerfumes() {
  ...
  const { rows } = await getPool().query(
    'SELECT id, name, price, image_url, video_url, description, stock, category, created_at FROM perfumes ORDER BY created_at DESC',
  );
  return rows;
}

export async function createPerfume({ name, price, imageUrl, videoUrl, description, category }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `INSERT INTO perfumes (name, price, image_url, video_url, description, category)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, name, price, image_url, video_url, description, stock, category, created_at`,
    [name, price, imageUrl, videoUrl || null, description || null, category || null],
  );
  return rows[0];
}

export async function updatePerfume(id, { name, price, imageUrl, videoUrl, description, category }) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `UPDATE perfumes
     ...
     WHERE id = $7
     RETURNING id, name, price, image_url, video_url, description, stock, category, created_at`,
    [name, price, imageUrl, videoUrl || null, description || null, category || null, id],
  );
  return rows[0];
}
...
export async function getSummary(role) {
  ...
  const [
    ...
    expensesTotal,
      ...
      pool.query(`
        SELECT
          (SELECT COUNT(*) FROM pandero_entries WHERE paying = true)::int AS paying_count,
          (SELECT COALESCE(SUM(cuotas_total), 0) FROM pandero_groups) AS locked_total
      `),
  ...
  // Gastos extras (envío, empaque, publicidad, etc.) — no están ligados a un
  // perfume ni a una compra, así que se restan aparte para llegar a la
  // ganancia neta real del negocio.
  const expensesTotalAmount = Number(expensesTotal.rows[0].total);
  const netProfit = grossProfit - expensesTotalAmount;
  ...
  return {
    ...
    // Distinto de panderoTotal: ese viene de ventas con payment_type = 'pandero'.
    // Esto es el ingreso por cuotas semanales del roster de panderos (quién le
    // toca cada perfume), que se cobra aparte y no es una venta. Se suma lo ya
    // cobrado y cerrado de semanas anteriores (cuotas_total, que queda fijo al
    // entregar el turno de la semana) más lo marcado "Pagando" en la semana
    // actual (todavía no se cierra hasta que se entregue el turno).
    panderoCuotasCount: panderoRosterPaying.rows[0].paying_count,
    panderoCuotasTotal:
      Number(panderoRosterPaying.rows[0].locked_total) +
      panderoRosterPaying.rows[0].paying_count * PANDERO_CUOTA_AMOUNT,
...
export async function listOrders() {
  await ensureSchema();
  const { rows } = await getPool().query(`
    SELECT o.id, o.perfume_id, o.customer_name, o.quantity, o.note, o.order_code, o.fulfilled, o.created_at,
...
export async function setPanderoEntryFulfilled(id, fulfilled, { soldByRole, soldByName } = {}) {
  ...
  // grupo: lo ya marcado como "Pagando" se suma al total histórico de cuotas
  // cobradas del grupo (para que no desaparezca del Resumen) y todos vuelven
  ...
  try {
    ...
    const { rows: entryRows } = await client.query(
      `UPDATE pandero_entries SET fulfilled = true WHERE id = $1
       RETURNING id, fulfilled, group_id, perfume_id, customer_name`,
      [id],
    );
    const entry = entryRows[0];
    if (entry) {
      await client.query(
        `UPDATE pandero_groups SET cuotas_total = cuotas_total +
           (SELECT COUNT(*) FROM pandero_entries WHERE group_id = $1 AND paying = true) * $2
         WHERE id = $1`,
        [entry.group_id, PANDERO_CUOTA_AMOUNT],
      );
      ...
      // registra como venta (payment_type 'pandero', igual que una venta
      // pagada por pandero) para que la vendedora gane su comisión por el
      // precio del perfume entregado, igual que en cualquier otra venta. Como
      // ya se cobró por adelantado con las cuotas semanales (no con abonos de
      // crédito normales), se registra de una vez como cobrada al 100% para
      // que la comisión quede disponible para pagar desde ya.
      const { rows: perfumeRows } = await client.query(
        'SELECT price FROM perfumes WHERE id = $1 FOR UPDATE',
        [entry.perfume_id],
      );
      ...
      if (perfume) {
        ...
        if (soldByRole === 'vendedora') {
          const percent = await getCommissionPercent();
          commissionAmount = Math.round(Number(perfume.price) * percent) / 100;
        }
        const { rows: saleRows } = await client.query(
          `INSERT INTO sales (perfume_id, quantity, unit_price, payment_type, customer_name, sold_by_role, sold_by_name, delivered, commission_amount)
        ...
        await client.query(
          ...
          [saleRows[0].id, perfume.price, 'Cobrado mediante cuotas del pandero'],
        );
        await client.query('UPDATE perfumes SET stock = GREATEST(stock - 1, 0) WHERE id = $1', [
          entry.perfume_id,
        ]);
      }
    }
    await client.query('COMMIT');
    return entry ? { id: entry.id, fulfilled: entry.fulfilled } : undefined;
  } catch (error) {
    await client.query('ROLLBACK');
    throw error;
  } finally {
...
export async function listPanderoGroups() {
  ...
  const { rows: entries } = await getPool().query(`
    SELECT e.id, e.group_id, e.position, e.customer_name, e.perfume_id, e.fulfilled, e.paying, e.created_at,
           f.name AS perfume_name,
