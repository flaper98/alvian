...
export async function updateWebOrderAction(id, prevState, formData) {
  ...
  try {
    ...
    // Al marcarlo como pagado se registra solo como venta. Si falla (por
    // ejemplo, sin stock) no se cambia el estado.
    if (await shouldAutoRegisterWebOrder(id, status)) {
